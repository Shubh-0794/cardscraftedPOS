...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header id="pos-header" className="px-3.5 sm:px-5 pt-3 sm:pt-4 pb-2 flex flex-col gap-2.5 sm:gap-3">
      {/* Top row: Title + Green sync status + Controls */}
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="flex flex-col">
            <h1 className="text-2xl sm:text-3xl font-bold text-blue-400 font-cursive tracking-wide leading-tight truncate">
              {cleanStoreName}
            </h1>
          ...
          <button
            ...
            className="flex items-center gap-1.5 mt-0.5 hover:opacity-80 transition-opacity text-left cursor-pointer"
            title="Supabase Database Connected - Click to manage"
          >
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isSyncing ? 'bg-amber-400 animate-spin' : 'bg-emerald-400 animate-pulse'}`}></span>
            <span className="text-[9.5px] sm:text-[10px] font-bold text-emerald-400 tracking-wider uppercase font-mono flex items-center gap-1">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="p-2 sm:p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#131e38] transition-colors cursor-pointer"
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#131e38] transition-colors cursor-pointer"
      ...
      <div className="w-full bg-[#0a101d] border border-[#1b2b48] rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 flex items-center justify-between text-xs text-slate-300 font-medium">
        <span className="font-semibold text-slate-200 text-[11px] sm:text-xs">
          {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </span>
        <span className="text-slate-400 text-[10px] sm:text-[11px] font-medium flex items-center gap-1.5">
